...
class error404 {
    ...
    private COMMANDS: TCommands = {
        ...
        commands: 'Liste des commandes : <span class="red"> help</span> , <span class="red"> report</span> ,<span class="red"> exit</span>\n',
        cls: '',
    }
    ...
    constructor() {
        this.keyboard.focus()
        if (screen.width < 991) this.keyboard.addEventListener('keyup', (event: KeyboardEvent): void => this.key(event))
        else document.addEventListener('keypress', (event: KeyboardEvent): void => this.key(event))
        document.addEventListener('keydown', (event: KeyboardEvent): void => this.backSpace(event))
